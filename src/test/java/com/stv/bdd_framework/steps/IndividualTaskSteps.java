...
//Сценарий 1
    ...
    public void sign_in_page_is_loaded() throws InterruptedException {
        ...
        Waiters.waitForElementVisible(getDriver(),2,new LoginPage().getLoginContainer()); //или можно использовать класс Waiters из пакета utils
...
//Сценарий 2
    ...
    public void user_inputs_password(String string) throws InterruptedException {
            Thread.sleep(1000);
            getDriver().findElement(By.id("LogOnModel_Password")).sendKeys(string); //вводим через клавиатуру значения пароля из таблицы в сценарии аутлайн в поле ввода пароля
    }

    @And("user clicks on checkbox Show Password")
    public void user_clicks_on_show_password_to_enable() throws InterruptedException {
            Thread.sleep(1000);
            getDriver().findElement(By.id("passwordToggle")).click(); //кликаем на чекбокс "показать пароль"
}

    @And("checkbox is switched on")
    public void isCheckboxSwitchedOn() throws InterruptedException {
            Thread.sleep(1000);
            Assert.assertTrue(getDriver().findElement(By.id("passwordToggle")).isEnabled()); //проверяем, включен ли чекбокс
    }

    @And("user clicks on checkbox to disable it")
    public void user_clicks_on_show_password_to_disable() throws InterruptedException {
            Thread.sleep(1000);
            getDriver().findElement(By.id("passwordToggle")).click(); //кликаем на чекбокс, чтобы его отключить
    }
    ...
    public void user_deletes_input_from_password_field() throws InterruptedException {
          ...
          // getDriver().findElement(By.id("LogOnModel_Password")).clear(); не подходит, т.к. после метода clear() не появляется валидационное сообщение, то есть необходимо удалять с клавиатуры
    }
    ...
    public void user_clicks_on_sign_in_securely_button() {
        getDriver().findElement(By.id("qa-login")).click(); //кликаем на кнопку sign in securely
    }
